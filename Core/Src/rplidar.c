...
bool RPLIDAR_StartScan(rplidar_measurement_t measurement[], uint32_t count,
	...
	return rpl_usr_buf != NULL ?
			_WaitForMultiResponse(RESPONSE_SCAN, timeout) : true;
}

bool RPLIDAR_StartScanExpress(rplidar_dense_measurements_t *measurements,
		uint32_t count, uint32_t timeout) {
	uint8_t packet[9] = { START_FLAG, REQ_SCAN_EXPR, 0x05, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x22 };
	...
	// If user didn't provide a buffer, return immediatly and the callback will be called
	return rpl_usr_buf != NULL ?
			_WaitForMultiResponse(RESPONSE_SCAN_EXPRESS, timeout) : true;
}
...
bool RPLIDAR_RequestHealth(rplidar_health_t *health, uint32_t timeout) {
	...
	// If user didn't provide a buffer, return immediatly and the callback will be called
	return rpl_usr_buf != NULL ?
			_WaitForResponse(RESPONSE_HEALTH, timeout) : true;
}
...
bool RPLIDAR_RequestSampleRate(rplidar_samplerate_t *samplerate,
	...
	// If user didn't provide a buffer, return immediatly and the callback will be called
	return rpl_usr_buf != NULL ?
			_WaitForResponse(RESPONSE_SAMPLERATE, timeout) : true;
}
...
__attribute__((weak)) void RPLIDAR_OnSampleRate(
		rplidar_samplerate_t *samplerate) {
	return;
}

__attribute__((weak)) void RPLIDAR_OnConfiguration(
		rplidar_configuration_t *config) {
	return;
}

__attribute__((weak)) void RPLIDAR_OnSingleMeasurement(
		rplidar_measurement_t *measurement) {
	return;
}

__attribute__((weak)) void RPLIDAR_OnDenseMeasurements(
		rplidar_dense_measurements_t *measurement) {
	return;
}
...
static bool _ParseResponse(uint8_t *response, uint16_t size) {
	...
	case RESPONSE_SAMPLERATE:
		...
		break;
	case RESPONSE_CONF: {
		rplidar_configuration_t config = { .type = response[3] << 24
				| response[2] << 16 | response[1] << 8 | response[0],
				.payload_size = size - 4, .payload = response + 4, };
		if (rpl_usr_buf != NULL) {
			// Use user buffer
			memcpy(rpl_usr_buf, &config, sizeof(rplidar_configuration_t));
			// Copy payload to end of user buffer
			memcpy(((rplidar_configuration_t*) rpl_usr_buf)->payload,
					response + 4, size - 4);
			rpl_last_complete_resp = RESPONSE_SAMPLERATE;
		} else {
			// Use callback
	...
	case RESPONSE_SCAN:
		if (size == sizeof(rplidar_measurement_t)) {
			rplidar_measurement_t *measurement =
					(rplidar_measurement_t*) response;
	...
	case RESPONSE_SCAN_EXPRESS:
		if (size == sizeof(rplidar_dense_measurements_t)) {
			rplidar_dense_measurements_t *measurements =
					(rplidar_dense_measurements_t*) response;
			if (rpl_usr_buf != NULL) {
				// Use user buffer
				if (rpl_multiresp_remaining > 0) {
					// Do not copy more than the needed number of measurements to prevent overflowing the user buffer
...
static bool _SendRequest(uint8_t *data, uint16_t size, bool resp) {
	...
	if (resp) {
		...
		if (HAL_UARTEx_ReceiveToIdle_DMA(rpl_uart, rpl_rx_buf, BUFFER_RX_SIZE)
				!= HAL_OK) {
			return false;
		}
	}
